...
// ============================================================
// LangGraph 多 Agent 聊天助手
// ============================================================
import { runSupervisorGraph } from './agent/supervisor';
import { IndexedDBSaver } from './agent/checkpointer';
import type { AgentEvent } from './agent/types';
import { AIMessage } from '@langchain/core/messages';

// Checkpointer 单例（延迟初始化，在渲染进程中通过 IndexedDB 创建）
let _checkpointer: IndexedDBSaver | null = null;

function getCheckpointer(): IndexedDBSaver {
  if (!_checkpointer) {
    _checkpointer = new IndexedDBSaver();
  }
  return _checkpointer;
}

/**
 * LangGraph 多 Agent 对话入口
 * 前端通过 IPC 调用此 handler
 */
ipcMain.handle('langgraph-chat', async (event, userInput: string, sessionId: string = 'default') => {
  try {
    const checkpointer = getCheckpointer();
    const mainWindow = BrowserWindow.fromWebContents(event.sender);

    // 事件回调：推送 Agent 工作流事件到 renderer
    const emitEvent = (agentEvent: AgentEvent) => {
      mainWindow?.webContents.send('agent-event', agentEvent);
    };

    const result = await runSupervisorGraph(userInput, sessionId, checkpointer, emitEvent);

    // 提取最终回复
    const finalResponse = result.finalResponse || (() => {
      // 从 messages 中找最后一条 AIMessage
      const lastAIMessage = [...result.messages].reverse().find(m => m instanceof AIMessage);
      return lastAIMessage ? (typeof lastAIMessage.content === 'string' ? lastAIMessage.content : '') : '';
    })();
    ...
    console.error('[langgraph-chat] 错误:', error);
    return {
      success: false,
      answer: `Agent 执行出错: ${error.message}`,
      agentEvents: [],
    };
  }
});
 ...
 * 清空指定会话的 checkpoint
 */
ipcMain.handle('langgraph-clear-session', async (_, sessionId: string = 'default') => {
  try {
    await getCheckpointer().deleteThread(sessionId);
    return { success: true };
  } catch (error: any) {
    console.error('[langgraph-clear-session] 错误:', error);
    return { success: false, error: error.message };
  }
});