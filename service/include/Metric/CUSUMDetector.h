 ...
 */
struct CUSUMConfig {
    double mu = 0.0;                    // 样本外预期均值（通常取训练期均值或 0）
    double sigma = 1.0;                 // 样本外预期波动率（用于计算 k 和 h）
    double lambda = 0.5;                // 容许偏差倍数 (k = lambda * sigma)
    double threshold_multiplier = 4.0;  // 阈值倍数 (h = threshold_multiplier * sigma * sqrt(N))
    size_t min_obs = 30;                // 最少观测数，低于此值不触发变点
};
...
struct CUSUMStepResult {
    bool change_point = false;          // 是否检测到变点
    size_t step_index = 0;              // 当前步索引
    double cusum_positive = 0.0;        // 正向累积和 S+
    double cusum_negative = 0.0;        // 负向累积和 S-
    double current_drift = 0.0;         // 当前净漂移量 (S+ - S-)
};
 ...
 */
struct CUSUMResult {
    std::vector<CUSUMStepResult> steps; // 每步检测结果
    size_t total_change_points = 0;     // 总变点次数
    double max_drift = 0.0;             // 最大漂移量
    size_t last_change_index = 0;       // 最后变点索引（0 表示未触发）
};
...
public:
    ...
    // === 只读访问器 ===
    bool has_change_point() const { return _last_result.change_point; }
