        ...
        if (node.nodes.size() > 1) {
            ...
            if (argsNode->name == "Arguments" && !argsNode->nodes.empty()) {
                ...
                if (argsNode->nodes.size() > 1) {
                    ...
                    if (auto num = tryExtractNumber(secondArg)) {
                        csNode.param = *num;
                    }
                }
            }
...
List<Pair<symbol_t, TradeAction>> FormulaParser::envokeMixedCase(const Vector<symbol_t>& symbols, const Set<String>& variantNames, DataContext& context) {
    ...
    // [DEBUG E] 打印每个 symbol 的最终决策（仅 BUY parser）
    if (_default == TradeAction::BUY) {
        static int eCounter = 0;
        if (++eCounter % 100 == 1) {
            int nBuy = 0, nHold = 0;
            for (auto& [sym, act] : decisions) {
                if (act == TradeAction::BUY) nBuy++;
                else nHold++;
            }
            INFO("[DEBUG E] BUY parser #{} buy={} hold={} total={}",
                 eCounter, nBuy, nHold, decisions.size());
            // Print first 8 decisions with eval result type
            int n = 0;
            for (auto& [sym, act] : decisions) {
                if (n >= 8) break;
                context_t val = evalNode(sym, *_ast, context);
                String typeName;
                if (std::holds_alternative<bool>(val)) typeName = "bool=" + std::string(std::get<bool>(val) ? "true" : "false");
                else if (std::holds_alternative<double>(val)) typeName = "double=" + std::to_string(std::get<double>(val));
                else typeName = "other";
                INFO("[DEBUG E]   sym={} action={} eval_type={}",
                     get_symbol(sym), (int)act - 1, typeName);
                n++;
            }
        }
    }

    // [DEBUG D] 输出所有 symbol 的 topk 节点 outputs（如果是 TOPK 调用）
    if (_default == TradeAction::BUY) {
            ...
            for (auto& [astAddr, nodeId] : _csAstNodeToId) {
                ...
                for (auto& [sym, val] : it->second.outputs) {
                    if (std::holds_alternative<bool>(val)) {
                        if (std::get<bool>(val)) nTrue++; else nFalse++;
                    }
                }
                INFO("[DEBUG D]     true_count={} false_count={}", nTrue, nFalse);
                // Print first 5 outputs
                int n = 0;
                for (auto& [sym, val] : it->second.outputs) {
                    if (n >= 5) break;
                    String s;
                    if (std::holds_alternative<bool>(val)) s = std::get<bool>(val) ? "true" : "false";
                    else if (std::holds_alternative<double>(val)) s = std::to_string(std::get<double>(val));
                    else s = "?";
                    INFO("[DEBUG D]     sym={} val={}", get_symbol(sym), s);
                    n++;
                }
            }
        }
