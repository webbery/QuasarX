...
List<QNode*> parse_strategy_script_v2(const nlohmann::json& content, Server* server, SlippageConfigInfo* outSlippageConfig, std::map<uint32_t, nlohmann::json>* outNodeConfigMap) {
    ...
    for (auto& edge: edges) {
        ...
        // EMD 边 data.imfIndex：sourceHandle 以 "-IMF" 结尾且 data.imfIndex
        ...
        if (boost::algorithm::ends_with(sourceHandle, "-IMF") &&
            ...
            // 兼容 camelCase (前端 JSON) 和 snake_case
            const char* imf_key = nullptr;
            if (edge["data"].contains("imfIndex")) imf_key = "imfIndex";
            else if (edge["data"].contains("imf_index")) imf_key = "imf_index";
            if (imf_key && edge["data"][imf_key].is_number_integer()) {
                int idx = edge["data"][imf_key].get<int>();
                if (idx >= 0) {
                    sourceHandle = from + "-nimf_" + std::to_string(idx);
                }
            }
        }
